...
#                   :after => { :tag => 'h3', :content => 'All Assigned Roles' }
  ...
  def test_should_redirect_to_list_on_GET_show_without_id
    ...
    assert_equal 'This user could not be found.', flash[:error]
  end
  ...
  def test_should_create_new_nonsaved_record_and_display_form_on_GET_create
    get :create
  ...
  def test_create_with_valid_data_on_create_POST
...
#    assert_template 'create'
  ...
  def test_should_display_update_form_on_invalid_state_transition
    assert_equal @icarus.id, @icarus_user.id
    assert_equal @icarus.login, @icarus_user.login
    assert_equal @icarus.state, @icarus_user.state
    assert_equal @icarus.email, @icarus_user.email
  ...
  def test_delete_say_yes
    assert_not_nil User.find(@zeus_user.id)

    post :delete, :id => @zeus_user.id, :yes => 'Yes'
    assert_response :redirect
    assert_redirected_to :action => 'list'
  ...
  def test_delete_say_no
    assert_not_nil User.find(@zeus_user.id)

    post :delete, :id => @zeus_user.id, :no => 'No'
    assert_response :redirect
  ...
  # and the hint is displayed in the user update form.
