...
class AnonymousUser
  ...
  def login
    ActiveRbacConfig.config :anonymous_user_login
  end
  
  # Returns the email adress of the anonymous user as configured.
  def email
    ActiveRbacConfig.config :anonymous_user_email
  end
  ...
  def roles
    ...
    @roles = ActiveRbacConfig.config(:anonymous_user_roles).collect do |role_title|
  ...
  def groups
    ...
    # fetch the role objects for the configured role titles otherwise
    @groups = ActiveRbacConfig.config(:anonymous_user_groups).collect do |groups_title|
